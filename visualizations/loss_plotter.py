...
folders = ['clip-0.98data', 'naiveclip-0.98data', '0.98data', 'lr0.00clip-0.98data']
labels = ['Clipping:\nProposed Bounds', "Clipping:\nBaseline Bounds", 'No Clipping', 'Zero LR']
colors = ['#FF5733', '#007acc',  '#333333', '#f01abe']
markers = ['o', '^', 's', 'd']

for label, folder, color, marker in zip(labels, folders, colors, markers):
    df = load_data(folder)
    ax = add_to_plot(ax, df, 'loss', label, color, marker)
...
fig.savefig('visualizations/loss_comparison.png', bbox_inches='tight', dpi=600)