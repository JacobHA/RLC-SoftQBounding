...
class SoftQAgent(BaseAgent):
    ...
    def _initialize_networks(self):
        # initialize the lower and upper bound networks:
        self.lower_bound_net = SoftQNet(self.env,
                                        hidden_dim=self.hidden_dim,
                                        device=self.device,
                                        perceptron=self.perceptron_model)
        
        self.upper_bound_net = SoftQNet(self.env,
                                        hidden_dim=self.hidden_dim,
                                        device=self.device,
                                        perceptron=self.perceptron_model)
    ...
    def gradient_descent(self, batch, grad_step: int):
        ...
        clipped_curr_softq = torch.clamp(curr_softq, max=ub_net, min=lb_net)
        ...
        if 'new_algo' in self.clip_method:
            if clip_loss > 0:
                loss = self.soft_weight * torch.nn.functional.huber_loss(clipped_curr_softq.squeeze(2), 
                                                        curr_softq, 
                                                        reduction='mean')
            else:
                loss = 0.5*sum(self.loss_fn(softq, expected_curr_softq)
